...
class TestBatchRotatingKVCacheState(unittest.TestCase):
    ...
    def test_update_eval_preserves_pre_decode_mask_snapshot(self):
        """Mask captured before a decode step must reflect pre-decode state,
        even if evaluated after the decode step completes."""
        cache = BatchRotatingKVCache(max_size=4, left_padding=[2, 0])
    ...
    def test_returned_tensor_eval_keeps_batch_rotating_memory_close_to_explicit_padding_eval(
        ...
        """Peak memory when only evaluating the returned tensor (relying on
        mx.depends) must stay within 2.0x of baseline where left_padding and
        offset are explicitly evaluated each step."""
